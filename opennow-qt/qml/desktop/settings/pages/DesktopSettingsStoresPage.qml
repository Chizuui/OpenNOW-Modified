...
DesktopSettingsPanel {
    ...
    Item {
        ...
        DesktopSettingsButton { anchors.right: parent.right; anchors.rightMargin: 16; anchors.verticalCenter: parent.verticalCenter; text: qsTr("Refresh status"); enabled: ShellStore.gameAccountsState !== "loading"; onClicked: ShellStore.refreshGameAccounts() }
    }
    Repeater {
        ...
        delegate: DesktopSettingsRow {
            ...
            DesktopSettingsButton { width: 92; text: status.action; compact: true; primary: Boolean(status.primary); enabled: !ShellStore.syncOperation && ShellStore.gameAccountsState !== "loading" && (modelData.supportsLinking || modelData.supportsSync); onClicked: page.settingsScreen.runStoreAction(modelData) }
        }
    }
    Text {
