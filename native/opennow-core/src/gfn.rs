...
impl GfnService {
    ...
    fn fetch_chizui_session(
        ...
        if !response.status().is_success() {
            return Err(ServiceError::response("ChizuiLogin session request failed", response));
        }
        let payload = response.json::<Value>().map_err(|error| {
            ServiceError::network("Invalid ChizuiLogin session response", error)
        })?;
        if payload["status"].as_str().is_some_and(|status| status != "success") {
            return Err(ServiceError::invalid(
                payload["error"].as_str().unwrap_or("ChizuiLogin session was rejected"),
            ));
        }
